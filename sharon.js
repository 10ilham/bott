...
const configuration = new Configuration({
    apiKey: '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',
});
...
// Fungsi Utama Ilham Munawar WA Bot
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (type === "notify" && !messages[0].key.fromMe) {
            try {
                ...
                const isMessageMentionBot = incomingMessages.includes("@6285172183259");
                ...
                if (!isMessageFromGroup) {
                    ...
                    if (incomingMessages.includes('siapa') && incomingMessages.includes('kamu')) {
                        await sock.sendMessage(
                            senderNumber,
                            { text: "Nama saya adalah Ilham Munawar. Saya adalah sebuah program AI (Artificial Intelligence), berdasarkan pada arsitektur GPT-3.5. Saya dirancang untuk dapat memberikan respons yang relevan dan berguna kepada pengguna yang ingin berkomunikasi dengan saya." },
                            { quoted: messages[0] },
                            2000
                        );
                    }else if (incomingMessages.includes('nama') && incomingMessages.includes('mu')) {
                        await sock.sendMessage(
                            senderNumber,
                            { text: "Saya Ilham" },
                            { quoted: messages[0] },
                    ...
                    }else if (incomingMessages.includes('dimana') && incomingMessages.includes('tinggal')) {
                        await sock.sendMessage(
                            senderNumber,
                            { text: "Saya tinggal di Garut, Jawa Barat." },
                            { quoted: messages[0] },
                            2000
                        );
                    }else if (incomingMessages.includes('alamat') && incomingMessages.includes('kamu')) {
                        await sock.sendMessage(
                            senderNumber,
                            { text: "Saya berada di Desa Pameungpeuk Kabupaten Garut, Provinsi Jawa Barat." },
                            { quoted: messages[0] },
                            2000
                        );
                    }else if (incomingMessages.includes('dimana') && incomingMessages.includes('kamu berada')) {
                        await sock.sendMessage(
                            senderNumber,
                            { text: "Saya berada di Desa Pameungpeuk Kabupaten Garut, Provinsi Jawa Barat." },
                            { quoted: messages[0] },
                    ...
                    }else if (incomingMessages.includes('dimana') && incomingMessages.includes('alamat mu')) {
                        await sock.sendMessage(
                            senderNumber,
                            { text: "Saya berada di Desa Pameungpeuk Kabupaten Garut, Provinsi Jawa Barat." },
                            { quoted: messages[0] },
